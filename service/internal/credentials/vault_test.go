...
func TestVaultListProjectTokens(t *testing.T) {
	tests := []struct {
		name           string
		admin          bool
		expectedTokens []types.ProjectToken
		mockVaultData  map[string]interface{}
		vaultErr       error
		errResult      bool
	}{
		{
			name:  "list project tokens success",
			admin: true,
			expectedTokens: []types.ProjectToken{
				{ID: "1234"}, {ID: "5678"},
			},
			mockVaultData: map[string]interface{}{
				"keys": []interface{}{"1234", "5678"},
			},
		},
		{
			name:           "list project tokens error",
			admin:          true,
			vaultErr:       errTest,
			errResult:      true,
			expectedTokens: []types.ProjectToken{},
			mockVaultData: map[string]interface{}{
				"errors": []interface{}{"some error"},
			},
		},
		{
			name:           "no project tokens for project",
			admin:          true,
			expectedTokens: []types.ProjectToken{},
			mockVaultData: map[string]interface{}{
				"errors": []interface{}{},
		...
		{
			name:      "list project tokens not admin",
			admin:     false,
			vaultErr:  errTest,
			errResult: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			projectTokens, err := v.ListProjectTokens("projectName")
			if err != nil {
				if !tt.errResult {
					t.Errorf("\ndid not expect error, got: %v", err)
				}
