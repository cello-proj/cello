    ...
    for line in lines:
        if line.strip():
            ...
            if len(values) >= 2:  # Ensure we have at least 2 columns
                ...
                if len(values) == 2:
                    row = {"project": values[0], "repository": values[1]}
                    data.append(row)
...
Note: If PGPASSWORD environment variable is not set, the script will securely prompt for the password.
    ...
    parser.add_argument(
        "db_host",
        help="PostgreSQL server hostname or IP address (optionally with :port)",
    )
    ...
    ORDER BY project;
    """
    projects_csv = run_psql_query(
        db_host, args.db_user, args.db_name, db_port, password, projects_query
    )
    ...
    """
    tokens_csv = run_psql_query(
        db_host, args.db_user, args.db_name, db_port, password, tokens_query
    )
